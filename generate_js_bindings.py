...
class JSBGenerate(object):
    ...
    def __init__(self, config):
        ...
        self.generate_as_cpp = config.generate_as_cpp
...
'''         
        ...
        fd.write(autogenerated_template % (os.path.basename(sys.argv[0]), os.path.basename(sys.argv[2]), datetime.date.today(), JSB_VERSION, name))
...
class JSBindings(object):
    ...
    def __init__(self, config, config_path=''):
        ...
        if len(config['generate_as_cpp']) and config['generate_as_cpp'][0].lower() == "yes":
            self.generate_as_cpp = config['generate_as_cpp']
        else:
            self.generate_as_cpp = None
